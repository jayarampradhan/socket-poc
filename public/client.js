...
socket.on('updatechat', function(username, data) {
	$('#conversation').append(
			'<b>' + escaped(username) + ':</b> ' + escaped(data) + "<br/>");
});
...
socket.on('updateusers', function(data) {
	...
	$.each(data, function(key, value) {
		$('#users').append(
				'<div><a href="' + searchUrlFor(key) + '" target="_blank">'
						+ key + '</div>');
	});
});
...
socket.on('servernotification', function(data) {
	...
	if (data.connected) {
		...
		$('#conversation').append(
				'connected: <a href="' + searchUrl + '" target="_blank">'
						+ escaped(data.username) + "</a><br/>");
	} else {
		$('#conversation').append(
				'disconnected: <a href="' + searchUrl + '" target="_blank">'
						+ escaped(data.username) + "</a><br/>");
	}
});
